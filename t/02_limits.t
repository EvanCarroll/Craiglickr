...
{
	local Craiglickr->config->{Craiglickr}{category}{cross_posting} = 0;
	like ( request('/craiglickr/locations/hou/boards/ctd,cto')->content, qr/Cross-posting.*?disabled/i, 'x-posting disabled (boards)' );
	local Craiglickr->config->{Craiglickr}{category}{cross_posting} = 1;
...
subtest 'No restrictions' => sub {
	...
	local Craiglickr->config->{Craiglickr}{category} = {cross_posting => 1, owner => 1, max => 100};
